...
window.findAndReplaceDOMText = (function() {
  ...
  /// Returns true if the node or any of its
  /// parents match an exclusion selector.
  function isExcluded(node) {
    if (!$.isArray(config.exclusions)) {
      return false;
    }
    node = $(node);
    for (var i = 0; i < config.exclusions.length; i++) {
      if (node.closest(config.exclusions[i]).length > 0)
        return true;
    }
    return false;
  }
   ...
   */
  function findAndReplaceDOMText(regex, node, replacementNode, captureGroup, options) {
    // Override default options where specified
    for (var opt in options || {}) {
      config[opt] = options[opt];
    }

    // Check exclusions list; return if necessary
    for (var i = 0; i < config.exclusions.length; i++) {
      if ($(node).is(config.exclusions[i])) {
        return;
      }
    }
  ...
  function _stepThroughMatches(node, matches, replaceFn) {
    ...
    out: while (true) {
      ...
      if (startNode && endNode) {
        // Only actually do the replacement if we're not on an excluded node.
        if (!isExcluded(curNode)) {
          curNode = replaceFn({
            startNode: startNode,
